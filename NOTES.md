...
**Q: Your agent was calling get_news and analyze_sentiment in the same step. Why is that a problem and how did you fix it?**

`analyze_sentiment` takes a list of news titles as input — titles that must come from `get_news`. When the LLM batches both calls in the same step, it hasn't seen the `get_news` results yet, so it fabricates titles to pass to `analyze_sentiment`. The sentiment scores in the report look real but are computed on hallucinated inputs. LangSmith made this visible — I could see both tools appearing under the same `tools` node with overlapping timings. The fix was `llm.bind_tools(tools).bind(parallel_tool_calls=False)` — this tells Groq to return one tool call at a time, forcing the agent to loop back through the agent node after each tool before deciding the next call. The agent now calls `get_stock_data` → `get_news` → `analyze_sentiment` → `get_earnings` → `retrieve_rag_chunks` in separate steps, each grounded in the previous result. I considered combining `get_news` and `analyze_sentiment` into a single tool, but that would remove the agent's flexibility to call either independently. `parallel_tool_calls=False` fixes the ordering problem without changing the tool interface.

---

**Q: You used LangSmith to debug your agent. What did you find, and how did it change your approach?**

When I first ran the LangGraph agent with RAG wired in, the output looked plausible — structured sections, specific numbers, red flags. But when I checked the LangSmith trace, `tool_calls` was empty on every run. The LLM was generating the entire report from its training data without calling a single tool. The numbers it cited — EPS, PE ratio, insider trades — were hallucinated. This was a significant finding because the system looked correct from the outside but was fundamentally broken. The root cause was the system prompt: it told the LLM to "produce a structured research report" without explicitly requiring it to gather real data first. The LLM saw the output format and satisfied it directly from memory. The fix is to add an explicit instruction that all tools must be called before the report is written. LangSmith also flagged that tool docstrings were describing implementation rather than telling the LLM when to call the tool — for example, `get_news` said "Takes in user ticker and finds top 10 articles" instead of "Call this tool first to retrieve recent news headlines for a ticker". LangSmith made this class of bug visible — without it, the agent would have appeared to work in production while silently hallucinating all its data.
